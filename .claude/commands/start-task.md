...
2. **GitHub 이슈 생성** — `gh issue create`로 제목 `유형: 설명`, 알맞은 라벨과 마일스톤을 달고, 본문은 이슈 템플릿 형식(무엇/왜/완료조건)으로 쓴다.
3. 생성된 **이슈 번호로 브랜치를 판다** — `유형/도메인/#이슈번호-설명` (예: `feat/map/#12-nearby-rpc`). `dev`에서 최신을 받아 분기한다:
