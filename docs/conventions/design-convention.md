...
아래 두 곳은 브라우저 밖(빌드 시점 이미지 생성·OS 크롬)에서 색을 읽어 **CSS 변수를 참조할 수 없다.** HEX 리터럴이 허용되는 유일한 예외이며, 그래서 **팔레트를 바꾸면 이 두 파일을 반드시 함께 갱신한다**(체크리스트).
...
`src/app/icon.svg` 도 같은 이유(정적 SVG 파일)로 리터럴이지만, 소스가 SVG 라 토큰 적용 자체가 불가능해 표에서 뺐다.
