...
아래는 HEX 리터럴이 허용되는 예외 전부다. 앞의 세 곳은 브라우저 밖(빌드 시점 이미지 생성·OS 크롬·정적 에셋)에서 색을 읽어 **CSS 변수를 참조할 수 없고**, 마지막은 테마와 무관하게 고정돼야 하는 색이다. **팔레트를 바꾸면 앞의 세 파일을 반드시 함께 갱신한다**(체크리스트 — species-icon 은 팔레트와 무관하므로 제외).
...
| `src/entities/species/ui/species-icon.tsx` | 종 캐릭터 일러스트의 고정색 — 강아지 털색은 라이트/다크를 따라가면 안 되는, 토큰이 아니라 그림의 일부다 |
