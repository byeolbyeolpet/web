...
**쿼리 형태가 RPC 안에 고정돼야 그 형태에 맞춰 인덱스를 짤 수 있다.** 클라이언트가 필터·정렬을 임의 조합으로 보내면 어떤 인덱스를 만들어야 할지 정할 수 없고, 결국 전부 seq scan 으로 떨어진다.

- 반경 검색처럼 인덱스가 성패를 가르는 것은 반드시 RPC (`nearby_places` 의 `ST_DWithin` + GIST).
- 복합 조건(카테고리 + 종 + 거리 + 상태)은 함수 하나로 받아 **복합 인덱스**를 맞춘다.
- **인덱스는 실측하고 넣는다.** `explain (analyze, buffers)` 로 실제로 타는지 확인한다. 주의: `analyze` 로 통계가 잡히기 전에는 플래너가 기본 추정치(550행)로 판단해 **안 쓸 인덱스도 쓰는 것처럼 보인다.** 작은 참조 테이블(`species` 14행)은 정렬조차 Seq Scan + Sort 가 이긴다 — 거기에 인덱스를 만들면 어드바이저에 `unused_index` 로만 남는다. ([ADR-0005](../adr/0005-species-code-system.md))
